...
describe TwemproxyExporter::Counter do
  ...
  context "#count" do
    it "should only increase" do
      expect do
        @counter.count(5)
      end.to change { @counter.send(:instance_variable_get, :@counter).get }.by(5)

      expect(@counter.send(:instance_variable_get, :@last)).to equal(5)

      expect do
        @counter.count(4)
      end.to change { @counter.send(:instance_variable_get, :@counter).get }.by(4)

      expect(@counter.send(:instance_variable_get, :@last)).to equal(4)

      expect do
        @counter.count(6)
      end.to change { @counter.send(:instance_variable_get, :@counter).get }.by(2)
